...
with open("morningstar_data.txt", "w", encoding="utf-8") as file:
	#format: each line has two components: the information as a long string, and the last character being the label
	for report in tqdm(morningstar_files):
		print(report)
		processed_text, label = process_input(os.path.join(path, report))
		file.write(processed_text + str(label)+"\n")
